...
| Command | Effect |
|---|---|
| `task:new --repo <r> --title <t> --class <c> --arm tri\|control [--issue <n> \| --pr <n>] [--kind implement\|pr_review\|research\|ops] [--base <sha>] [--branch <b>] [--worktree <path>] [--owner <id>] [--priority <n>] [--due <iso>] [--parent <task>] [--sibling <task>]` | Insert task, status `submitted`, print id. Refuses (exit 6, reason `archived`) when `--sibling`/`--parent` names an archived task -- a new task may not attach itself to one, see "Archive, never delete" below |
...
| `task:unarchive --task <id>` | Restore an archived task to `board`/`compare`/`report`, and to every command `checkNotArchived` gates (below) |
| `task:find --repo <r> [--pr <n>] [--arm tri\|control] [--kind implement\|pr_review\|research\|ops] [--class <c>] [--include-archived]` | Print matching task ids, one per line, newest-created last; empty output and exit 0 when nothing matches. Excludes archived tasks unless `--include-archived` is passed. This is the supported way to ask "is there already a task for this repo/PR/arm" -- a driver or orchestrator should call this instead of parsing `export` and filtering `archived_at` itself (the real Phase 1a defect: a driver that read `export` found and launched into an archived task -- see "Archive, never delete" below) |

Real Phase 1a defect and fix: `run:start` (and so `run:launch`), `review:brief`, `verdict`, `msg`, `artifact`, `test`, `intervene`, `task:close`, `adjudicate`, `task:resolve`, `task:reject`, and `task:new`'s `--sibling`/`--parent` linkage above all refuse **exit 6, reason `archived`** on an archived task, checked before any other state check -- see `docs/state-machine.md` "Archive, never delete" for the production defect this closes and the exact command list. `task:show`, `export`, `board`, and `preflight` are read-only/diagnostic and stay unaffected by archiving, as already documented above.
...
`--retry <n>` (default `0`) and `--retry-backoff-s <seconds>` (default `30`) opt `run:launch` into waiting for its own run to finish and relaunching it when the run ends `provider_unavailable` (`docs/state-machine.md` "Provider unavailable") -- omitting `--retry` entirely keeps the original fire-and-forget behaviour (returns immediately after spawn, no waiting, no retrying). With `--retry` given, each `provider_unavailable` outcome sleeps `backoff × 2^(attempt-1)` with full jitter, capped at 15 minutes, logs the wait to stderr, and relaunches the same task as a new run, up to `n` extra attempts. Every relaunch first archives whatever the previous attempt left in the shared `<runs>/<task>/<agent>/` out dir into `attempts/<previous run id>/` -- never deleted, recorded on the previous run's `task_runs.attempt_evidence_dir` -- so a retried attempt always starts from a clean out dir and the old attempt's own evidence stays on disk (`docs/state-machine.md` "Provider unavailable"). Exit 0 on any attempt that is not `provider_unavailable`. Exit 7 (`docs/state-machine.md`'s exit code table) with exactly one `warn` escalation, reason `provider_unavailable`, if every attempt (the first plus all `n` retries) ends `provider_unavailable` -- the escalation's detail names the attempt count; nothing is written per retry.
