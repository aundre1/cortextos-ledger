...
`cortexctl task:archive` is how a superseded or aborted review task (a reviewer run against the wrong base commit, a PR triage overtaken by a force-push, a batch abandoned mid-review) leaves the reviewed set without being deleted -- the task's row, its brief, diff, verdicts, and adjudication all stay exactly as they are. `review:brief`, `verdict`, and `adjudicate` all refuse on an archived task (exit 6, reason `archived`, checked before any other gate -- `docs/state-machine.md` "Archive, never delete") until `cortexctl task:unarchive` clears it, so a stale brief or a superseded verdict can never be written into a task the reviewer or a human has set aside. `report`'s reviewer-precision statistics exclude an archived task's verdicts entirely, the same exclusion applied to every other statistic; `cortexctl export` and `task:show` still carry the task and its verdicts in full, so a published triage result can always be audited against what was set aside and why.
