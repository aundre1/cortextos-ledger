...
| `<runs>/.opencode-data/<agent>/` | The per-agent isolated OpenCode data directory (docs/adapters.md "Concurrency note"): OpenCode's own session history, its own sqlite db, snapshots, and logs for one agent's runs. **Contains no credentials.** `auth.json` normally lives in this same kind of directory, but this kit never copies it here -- it is forwarded to the child process at launch via the `OPENCODE_AUTH_CONTENT` environment variable instead (docs/adapters.md "opencode: credential forwarding (D1)"), so nothing auth-shaped is ever written to disk under `<runs>` at all | Same lifetime as `<runs>` itself (never pruned by this kit). Never committed: it sits under `.cortex/runs` -- `.cortex/` is git-ignored in full and `scripts/publish-check.mjs` blocks any staged path under it -- and it is outside every task's own `worktree`, so `preflight`'s secrets scanner (which only ever walks a task's worktree) never has a reason to look at it |
| `<runs>/<task>/<agent>/attempts/<run id>/` | A superseded `run:launch --retry` attempt's own artifacts (`done.marker`, `exit.txt`, `elapsed_ms.txt`, `events.jsonl`, `out.txt`, `pid.txt`, `prompt.txt`, `patch.diff`, `reasoning.md`, `verdict.json` -- whichever it produced), moved out of the shared `<runs>/<task>/<agent>/` out dir by the next attempt so it can start clean without erasing this one's evidence (`docs/state-machine.md` "Provider unavailable"; `src/adapters/spawn.mjs`'s `archivePriorAttempt`) | Same lifetime and redaction as the files it holds -- every file already redacted at rest before archiving (`out.txt`/`prompt.txt` line by line as written, same as everywhere else) is moved, not copied unredacted then cleaned up. Same never-committed treatment as the rest of `<runs>` |
...
For OpenCode specifically, this is bound at runtime, not only documented: since OpenCode's own agents are defined in *its* config, not this kit's, `src/adapters/opencode.mjs` generates one at launch (`config.agents.<name>.opencode` or `.template`, translated into OpenCode's own `agent.<name>` schema -- see docs/adapters.md "opencode: agent definition and permission binding (D2)") and injects it via `OPENCODE_CONFIG_CONTENT`, and refuses to launch (exit 1) whenever `read_only: true` is set but the resulting permission block does not actually deny `edit`.
