...
{"ts":"...","type":"error","severity":"halt","status_code":429,"retryable":true,"message":"..."}
```

Adapters translate their harness's native stream into this format. `ingest` reads only this format. Never log tool arguments in full; `args_summary` is capped at 200 characters and secrets patterns are redacted before write. `error` reports a harness-level failure the harness itself surfaced mid-stream (a provider's 4xx or 5xx, for example); it is informational only and never overrides the run's real exit code, which stays authoritative wherever it is recorded (`ingest` reads it from the run row, not from this event).

All three real adapters (`opencode`, `claude`, `codex`) emit this event with `status_code` (a number, or `null` when the harness reported none) and `retryable` (a boolean, always present, never omitted). `retryable` is derived honestly, never invented: `opencode.mjs` reads it straight from the harness's own `error.data.isRetryable`/`error.isRetryable`; `claude.mjs` and `codex.mjs` have no documented error-stream shape from their harnesses (unlike OpenCode's, this is not verified against a real capture -- see the wave log's OPEN QUESTION), so both derive `retryable` only from a whitelist of HTTP statuses the harness itself reported (`429`, `500`, `502`, `503`, `504` -- `src/adapters/error-event.mjs`'s `retryableFromStatus`), and report `false` for every other status or for no status at all, rather than guessing. A run whose stream ends on one of these events with `retryable: true` and no successful completion is classified `task_runs.failure_class = 'provider_unavailable'` -- see `docs/state-machine.md` "Provider unavailable".

Every exit code an adapter or the runner persists (`task_runs.exit_code`) is passed through `normalizeExitCode()` (`src/exit-code.mjs`) first: a Windows child process that terminates as an unsigned 32-bit value (`4294967295`, for example) is converted back to the signed value a POSIX exit code would have been (`-1`), so the same failure reads the same way in the ledger regardless of which OS ran it.
